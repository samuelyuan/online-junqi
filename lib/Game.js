...
var getMovesForPlayer = function(playerColor, board, lastMove) {
  ...
  for (square in board) 
  {
    ...
    {
        moves.push.apply(moves, getMovesRailroad(piece, square, board));
    }
  }
 ...
 * Returns an array of move objects on success or an empty array on failure.
 */
var getMovesRailroad = function(piece, square, board) 
{
    var moves = [];
...
{    
    ...
    {
        return getNeighbors(currentSquare);
    }
    
    var singleRail = [];
    // get all positions that are accessible on the same railraod
