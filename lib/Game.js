...
var getAdjacentNeighbors = function(currentSquare, railroadArray)
{   
    ...
    if (bunkerSquares.indexOf(square) > -1) {
        var transforms = [
            {x:+0, y:+1}, {x:+1, y:+1},
            {x:+1, y:+0}, {x:+1, y:-1},
            {x:+0, y:-1}, {x:-1, y:-1},
    ...
    railroadArray.forEach(function(railroad) {
        allOuterLocations = allOuterLocations.concat(railroad);
    });
    
    // add back rows
    allOuterLocations = allOuterLocations.concat(['a1', 'c1', 'e1', 'a12', 'c12', 'e12']);
    allOuterLocations.forEach(function(square) {
        // the middle of the board is blocked for these squares
        if (square === 'b6' || square === 'd6' || square === 'b7' || square === 'd7') {
            return;
        }
        
        if (square in moveMap) {
            return;
        }
...
{    
    ...
    {
        return getAdjacentNeighbors(currentSquare, railroadArray);
    }
    ...
    while (reachableSquares.length != 0)
    {
        ...
        var neighbors = getAdjacentNeighbors(iterSquare, railroadArray);    
        ...
        neighbors.forEach(function(nextSquare) {
            ...
            {
                // not visited
                if ((nextSquare in isVisited) === false)
                {
                    var ENGINEER = "9";
    ...
    var neighbors = getAdjacentNeighbors(currentSquare, railroadArray);
    neighbors.forEach(function(square) {
        isVisited[square] = true;
    });
    
    return Object.keys(isVisited);
}
