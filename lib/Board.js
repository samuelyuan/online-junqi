...
Board.prototype.getSwapForAll = function() {
  ...
  for (square in this.boardState)
  {
    ...
    moves.push.apply(moves, getSwapMoves(square, this.boardState));
  }
...
var isBunkerSquare = function(square) {
    ...
    return bunkerSquares.includes(square);
}

var isSquareAttackable = function(piece, board, destination) {
    return (board[destination].getPieceColor() !== piece.getPieceColor() && isBunkerSquare(destination) === false);
}
...
var getSwapMoves = function(square, board) {
  var piece = board[square];
  var moves = [];
  var playerSquares = getAllPlayerSquares(board, piece.getPieceColor());
  var validSwapSquares = playerSquares
  ...
  return validSwapSquares.map(function(destination) {
    return {
      type          : 'swap',
      startSquare   : square,
      endSquare     : destination
