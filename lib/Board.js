 ...
 */
 Board.prototype.getMovesForPlayer = function(playerColor) {
   ...
   for (square in this.boardState)
   {
     ...
     if (piece.getPieceColor() !== playerColor[0]) { continue; }
...
var doesPieceExist = function(boardState, playerColor, pieceRank)
{
    ...
    for (var sq in boardState) {
        ...
        if (piece && piece.getPieceColor() == playerColor && piece.getRank() == pieceRank) {
...
var getValidMoves = function(piece, square, board)
{
    ...
    // bfs
    var pieceRank = piece.getRank();
    var isPieceEngineer = (pieceRank === RANK_ENGINEER);
    ...
    reachableSquares.forEach(function(destination) {
        ...
        if (board[destination] == null) {
          moves.push(getSafeMove(square, destination));
        }
        // If destination square is occupied by an enemey and the enemy is not in a bunker
          ...
          moves.push(getAttackMove(square, destination));
        }
    });
    return moves;
}

var getSafeMove = function(square, destination)
{
    return {
        type        : 'move',
        startSquare : square,
...
var getAttackMove = function(square, destination)
{
    return {
        type        : 'attack',
        startSquare : square,
...
var isSquareAttackable = function(piece, board, square) {
    return (board[square].getPieceColor() !== piece.getPieceColor() && isBunkerSquare(square) === false);
}
...
var getSwapMoves = function(piece, square, board) {
  var moves = [];
  nodeKeys.forEach(function(destination) {
    ...
    if (board[destination].getPieceColor() !== piece.getPieceColor()) {
      return;
    }
    ...
    var pieceRank = piece.getRank();
    if (isDestinationPositionValid(pieceRank, square, destination)) {
      moves.push(swapMove);
    }
  });
